function bpa = tapas_bayesian_parameter_average_modified(estmodel)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% 
% INPUT ARGUMENTS:
%     estmodel           Estimate structures generated by tapas_fitModel(...). Note that all estimates
...
prc_model = estmodel{1}.c_prc.model;
obs_model = estmodel{1}.c_obs.model;

% Get priors
prc_priormus = estmodel{1}.c_prc.priormus;
prc_priorsas = estmodel{1}.c_prc.priorsas;
obs_priormus = estmodel{1}.c_obs.priormus;
obs_priorsas = estmodel{1}.c_obs.priorsas;

% Check whether everything matches up
for i = 2:n
    if ~strcmp(prc_model,estmodel{i}.c_prc.model)
        error('tapas:hgf:bpa:PrcModNoMatch', 'Perceptual models do not match.');
    end

    if ~strcmp(obs_model,estmodel{i}.c_obs.model)
        error('tapas:hgf:bpa:ObsModNoMatch', 'Observation models do not match.');
    end

    if ~isequalwithequalnans(prc_priormus,estmodel{i}.c_prc.priormus) || ~isequalwithequalnans(prc_priorsas,estmodel{i}.c_prc.priorsas)
        error('tapas:hgf:bpa:PrcPriorsNoMatch', 'Perceptual priors do not match.');
    end

    if ~isequalwithequalnans(obs_priormus,estmodel{i}.c_obs.priormus) || ~isequalwithequalnans(obs_priorsas,estmodel{i}.c_obs.priorsas)
        error('tapas:hgf:bpa:ObsPriorsNoMatch', 'Observation priors do not match.');
    end

    if ~isequalwithequalnans(u(:),estmodel{i}.u(:))
...
bpa.u     = u;
bpa.ign   = [];
bpa.c_prc = estmodel{1}.c_prc;
bpa.c_obs = estmodel{1}.c_obs;
...
for i=1:n
    H = H + estmodel{i}.optim.H;
end

% Ensure H and Sigma are positive semi-definite
Sigma   = inv(H);
Sigma = (Sigma' + Sigma)./2;
% Corr    = tapas_Cov2Corr(Sigma); % not needed
...
for i=1:n
    mui = [estmodel{i}.p_prc.ptrans, estmodel{i}.p_obs.ptrans]';
    mui = mui(opt_idx);
    mu = mu + estmodel{i}.optim.H*mui;
end
